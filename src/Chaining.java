...
public class Chaining {
    ...
    public int chain (int key) {
        ...
        return (A*key)%power2(w) >> (w-r); //h function from assignment sheet
    }
        
    
    /**Inserts key k into hash table. Returns the number of collisions encountered*/
    public int insertKey(int key){
    	int collision =this.Table.get(chain(key)).size(); //size of the list of the list that given key belongs to get the number of collisions
    	this.Table.get(chain(key)).add(key); //adding given key to the list where it belongs 
        return collision;
